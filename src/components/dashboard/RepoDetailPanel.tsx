...
import { X, Star, GitFork, AlertCircle, GitCommit, ExternalLink } from 'lucide-react';
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (Array.isArray(activityData) && activityData.length > 0) {
          const formattedActivity = activityData.map(week => ({
    ...
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-github-dark border-l border-github-border z-50 shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
        <div className="flex items-start justify-between p-6 border-b border-github-border bg-github-canvas sticky top-0 z-10">
          <div>
            <h2 className="text-2xl font-black uppercase text-white tracking-tighter mb-1">{repo.name}</h2>
            <p className="text-sm font-mono text-github-muted">{repo.description || 'NO DESCRIPTION'}</p>
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-github-canvas sharp-border p-4 flex flex-col items-center">
              ...
              <span className="text-xl font-black text-white">{repo.stargazers_count}</span>
              <span className="text-xs font-mono text-github-muted uppercase">Stars</span>
            </div>
            <div className="bg-github-canvas sharp-border p-4 flex flex-col items-center">
              <GitFork className="w-4 h-4 text-[var(--color-aossie-green)] mb-2" />
              <span className="text-xl font-black text-white">{repo.forks_count}</span>
              <span className="text-xs font-mono text-github-muted uppercase">Forks</span>
            </div>
            <a 
            ...
            >
              ...
              <span className="text-xl font-black text-[var(--color-aossie-yellow)] flex items-center gap-1">
                {repo.open_issues_count} <ExternalLink className="w-3 h-3 text-github-muted opacity-0 group-hover:opacity-100 transition-opacity" />
              </span>
              <span className="text-xs font-mono text-github-muted uppercase">Issues</span>
            </a>
          </div>
          ...
          <div className="space-y-3">
            <h3 className="font-bold uppercase tracking-widest text-white text-sm flex items-center gap-2">
              <GitCommit className="w-4 h-4 text-[var(--color-aossie-green)]" /> Activity [52 WEEKS]
            </h3>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={activity}>
                    <Tooltip 
                      ...
                      contentStyle={{ backgroundColor: '#0d1117', borderColor: '#30363d', borderRadius: '0' }}
                      labelStyle={{ color: '#8b949e', fontFamily: 'monospace' }}
                      itemStyle={{ color: '#F7D100', fontFamily: 'monospace' }}
                    />
                    <Bar dataKey="commits" fill="#8CC63F" radius={[0, 0, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
          ...
          {/* Top Contributors */}
          <div className="space-y-3">
            <h3 className="font-bold uppercase tracking-widest text-white text-sm">Top Contributors</h3>
            {loading ? (
              <div className="space-y-3">
                {[1, 2, 3].map(i => (
                  <div key={i} className="flex items-center gap-3 animate-pulse bg-github-canvas sharp-border p-3">
                ...
                {contributors.map(contributor => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <img src={contributor.avatar_url} alt={contributor.login} className="w-10 h-10 sharp-border grayscale group-hover:grayscale-0 transition-all duration-300" />
                      <div>
